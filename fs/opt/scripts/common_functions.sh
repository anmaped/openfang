...
http_server() {
	...
	restart)
		...
		;;
	status)
		if pgrep lighttpd.bin &>/dev/null; then
			echo "ON"
		else
			echo "OFF"
		fi
...
rtsp_h264_server() {
	...
	off)
		...
		;;
	status)
		if /system/sdcard/controlscripts/rtsp-h264 status | grep -q "PID"; then
			echo "ON"
		else
			echo "OFF"
		fi
...
rtsp_mjpeg_server() {
	...
	off)
		...
		;;
	status)
		if /system/sdcard/controlscripts/rtsp-mjpeg status | grep -q "PID"; then
			echo "ON"
		else
			echo "OFF"
		fi
