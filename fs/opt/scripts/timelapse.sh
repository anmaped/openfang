...
TIMELAPSE_CONF='/opt/config/timelapse.conf'
BASE_SAVE_DIR='/opt/DCIM/timelapse'
...
while true; do
    ...
    if [ -z "$COMPRESSION_QUALITY" ]; then
         /usr/bin/getimage > "$SAVE_DIR/$filename" &
    else
        /usr/bin/getimage | /usr/bin/jpegoptim -m"$COMPRESSION_QUALITY" --stdin --stdout > "$SAVE_DIR/$filename" &
    fi
