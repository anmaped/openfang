...
if [ -n "$F_cmd" ]; then
  ...
  case "$F_cmd" in
    showlog)
      ...
      case "${F_logname}" in
        ...
        3)
            ...
            ;;
        4)
            echo "Contents of v4l2rtspserver.log<br/>"
            cat /var/log/v4l2rtspserver.log
            ;;
        5)
    ...
    clearlog)
      ...
      case "${F_logname}" in
        ...
        4)
            ...
            echo -n "" > /var/log/v4l2rtspserver.log
            ;;
        5)
    ...
    motor_left)
      ...
      SLEEP_NUM=$(awk -v a="$F_val" 'BEGIN{printf ("%i",a*10000)}')
      usleep ${SLEEP_NUM//-/}
      # Display AXIS to OSD
      update_axis
      /usr/bin/setconf -k o -v "$OSD"
    ;;
    ...
    motor_PTZ)
       /opt/scripts/PTZpresets.sh $F_x_axis $F_y_axis                        
    ;;
    ...
    h264_start)
      /opt/controlscripts/rtsp-h264 start
    ;;
    ...
    mjpeg_start)
      /opt/controlscripts/rtsp-mjpeg start
    ;;
...
#    ;;
    ...
    rtsp_stop)
      /opt/controlscripts/rtsp-h264-with-segmentation stop
      /opt/controlscripts/rtsp-mjpeg stop
      /opt/controlscripts/rtsp-h264 stop
    ;;
    ...
    osd)
      ...
      if [ ! -z "$axis_enable"];then
        ...
        echo "DISPLAY_AXIS=false" > /opt/config/osd.conf
        echo DISPLAY_AXIS disable
      fi

      if [ ! -z "$enabled" ]; then
        /system/sdcard/bin/setconf -k o -v "$osdtext"
        echo "OSD=\"${osdtext}\"" >> /opt/config/osd.conf
        echo "OSD set"
      else
        echo "OSD removed"
        /system/sdcard/bin/setconf -k o -v ""
        echo "OSD=\"\" " >> /opt/config/osd.conf
      fi

      echo "COLOR=${F_color}" >> /opt/config/osd.conf
      /system/sdcard/bin/setconf -k c -v "${F_color}"

      echo "SIZE=${F_size}" >> /opt/config/osd.conf
      /system/sdcard/bin/setconf -k s -v "${F_size}"

      echo "POSY=${F_posy}" >> /opt/config/osd.conf
      /system/sdcard/bin/setconf -k x -v "${F_posy}"

      echo "FIXEDW=${F_fixedw}" >> /opt/config/osd.conf
      /system/sdcard/bin/setconf -k w -v "${F_fixedw}"

      echo "SPACE=${F_spacepixels}" >> /opt/config/osd.conf
    ...
    setldravg)
      ...
      echo AVG="$ldravg" > /opt/config/ldr-average.conf
      echo "Average set to $ldravg iterations."
      return
    ;;

    auto_night_mode_start)
      /opt/controlscripts/auto-night-detection start
    ;;

    auto_night_mode_stop)
      /opt/controlscripts/auto-night-detection stop
    ;;
    ...
    flip-off)
      /usr/bin/setconf -k f -v 0
    ;;

    motion_detection_on)
        motion_sensitivity=4
        if [ -f /opt/config/motion.conf ]; then
            source /opt/config/motion.conf
        fi
        if [ $motion_sensitivity -eq -1 ]; then
             motion_sensitivity=4
        fi
        /usr/bin/setconf -k m -v $motion_sensitivity
        rewrite_config /opt/config/motion.conf motion_sensitivity $motion_sensitivity
    ;;
    ...
    set_video_size)
      ...
      rewrite_config /opt/config/rtspserver.conf RTSPH264OPTS "\"$video_size\""
      rewrite_config /opt/config/rtspserver.conf RTSPMJPEGOPTS "\"$video_size\""
      rewrite_config /opt/config/rtspserver.conf BITRATE "$brbitrate"
      rewrite_config /opt/config/rtspserver.conf VIDEOFORMAT "$video_format"
    ...
    set_region_of_interest)
        rewrite_config /opt/config/motion.conf region_of_interest "${F_x0},${F_y0},${F_x1},${F_y1}"
        rewrite_config /opt/config/motion.conf motion_sensitivity "${F_motion_sensitivity}"
        rewrite_config /opt/config/motion.conf motion_indicator_color "${F_motion_indicator_color}"
        rewrite_config /opt/config/motion.conf motion_timeout "${F_motion_timeout}"
        if [ "${F_motion_tracking}X" == "X" ]
        then
            rewrite_config /opt/config/motion.conf motion_tracking off
             /usr/bin/setconf -k t -v off
        else
            rewrite_config /opt/config/motion.conf motion_tracking on
            /usr/bin/setconf -k t -v on
        fi

        /usr/bin/setconf -k r -v ${F_x0},${F_y0},${F_x1},${F_y1}
        /usr/bin/setconf -k m -v ${F_motion_sensitivity}
        /usr/bin/setconf -k z -v ${F_motion_indicator_color}
        /usr/bin/setconf -k u -v ${F_motion_timeout}
    ...
    ;;
    offDebug)
        /opt/controlscripts/debug-on-osd stop

    ;;
    onDebug)
        /opt/controlscripts/debug-on-osd start
    ;;
    ...
    conf_timelapse)
      ...
      if [ "$tlinterval" ]; then
        rewrite_config /opt/config/timelapse.conf TIMELAPSE_INTERVAL "$tlinterval"
      ...
      if [ "$tlduration" ]; then
        rewrite_config /opt/config/timelapse.conf TIMELAPSE_DURATION "$tlduration"
    ...
    conf_audioin)
       ...
       rewrite_config /opt/config/rtspserver.conf AUDIOFORMAT "$audioinFormat"
       rewrite_config /opt/config/rtspserver.conf AUDIOOUTBR "$audioinBR"
       rewrite_config /opt/config/rtspserver.conf FILTER "$F_audioinFilter"
       rewrite_config /opt/config/rtspserver.conf HIGHPASSFILTER "$F_HFEnabled"
       rewrite_config /opt/config/rtspserver.conf HWVOLUME "$F_audioinVol"
       rewrite_config /opt/config/rtspserver.conf SWVOLUME "-1"
       ...
       /usr/bin/setconf -k q -v "$F_audioinFilter" 2>/dev/null
       /usr/bin/setconf -k l -v "$F_HFEnabled" 2>/dev/null
       /usr/bin/setconf -k h -v "$F_audioinVol" 2>/dev/null
       return
     ;;
     update)
